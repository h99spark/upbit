...
class Kiwoom(QAxWidget):
    ...
    def get_account_num(self):
        account_list = self.dynamicCall("GetLoginInfo(str)", "ACCNO")  # 계좌번호 리스트 반환
        self.account_num = account_list.split(';')[0]
        print(f"계좌번호 : {self.account_num} ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')})")
    ...
    def get_deposit(self):
        ...
        self.dynamicCall("SetInputValue(str, str)", "조회구분", "1")
        self.dynamicCall("CommRqData(str, str, str, str)", "예수금상세현황요청", "opw00001", '0', self.screen_deposit)

        self.get_deposit_event_loop.exec_()

    def read_csv_file(self):
        data = pd.read_csv("C:\\Users\\박성훈\\Documents\\highest.csv")
    ...
    # def detail_account_mystock(self):
    #     self.dynamicCall("SetInputValue(QString, QString)", "계좌번호", self.account_num)
    #     self.dynamicCall("SetInputValue(QString, QString)", "비밀번호", "0000")
    #     self.dynamicCall("SetInputValue(QString, QString)", "비밀번호입력매체구분", "00")
    #     self.dynamicCall("SetInputValue(QString, QString)", "조회구분", "1")
    #     self.dynamicCall("CommRqData(QString, QString, int, QString)", "계좌평가잔고내역요청", "opw00018", '0', self.screen_deposit)
    #
    #     self.get_deposit_event_loop.exec_()
    ...
    def trdata_slot(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
        ...
        if sRQName == "예수금상세현황요청":
            self.deposit = int(self.dynamicCall("GetCommData(str, str, str, str)", sTrCode, sRQName, '0', "예수금"))
            print(f"예수금 : {self.deposit} ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')})")

            self.stop_screen_cancel(self.screen_deposit)
            self.get_deposit_event_loop.exit()
            ...
            print(code, start_date, fibo_start_price)
